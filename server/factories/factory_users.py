...
class SupportedLocalesFactory(factory.django.DjangoModelFactory):
    """Factory for creating SupportedLocales instances with predefined locale codes ('en-US', 'pt-BR')."""
...
class UserFactory(factory.django.DjangoModelFactory):
    """Factory for generating User instances with randomized usernames, emails, first names, and activation statuses."""
    ...
    date_joined = factory.LazyAttribute(
        lambda o: random_datetime_last_30_days(o.date_now_ref)
    )


class UserProfileFactory(factory.django.DjangoModelFactory):
    """Factory for producing UserProfile instances associated with User instances and locales generated by SupportedLocalesFactory."""
...
class PendingUserFactory(factory.django.DjangoModelFactory):
    """Factory for creating PendingUser instances representing users who have registered but not yet confirmed their email. Includes a fake UUID as an email token."""
...
class RecoverPasswordFactory(factory.django.DjangoModelFactory):
    """Factory for generating RecoverPassword instances for users requesting password resets, using a defined reference date and setting an expiration date 5 days post-request."""
    ...
    user = factory.SubFactory(UserFactory)
    email_token = factory.Faker("uuid4")
    requested_date = factory.LazyAttribute(
        lambda o: random_datetime_last_30_days(o.date_now_ref)
    )
...
def create_user(n_users, seed, date_now_ref):
    """Generates a specified number of User instances with repeatable data generation using a fixed seed."""
    random.seed(seed)
    Faker.seed(seed)
...
def create_user_profile(users, locales):
    """Generates UserProfiles for given users, each associated with a randomly chosen locale."""
    user_profiles = []
    for user in users:
...
def create_recover_password(users, n_users, seed, date_now_ref):
    """Generates password recovery instances for a sample of users using a fixed seed for consistency."""
    random.seed(seed)
    Faker.seed(seed)
...
def generate_data_user(n_user, n_recover, date_now_ref):
    """Main function to generate comprehensive user-related data and prepare it for export, ensuring repeatable results with fixed seeding."""
